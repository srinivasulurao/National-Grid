	...
	protected function filtersToSearchArgs()
    {
		...
		$Uri_result=explode('/',$_SERVER['REQUEST_URI']);
			...
			if($Uri_result[2]=='supplier_feedback' && $ci->session->getSessionData('complaint_filter')=="search_text")
			{
				...
				if(!empty($srch))
				{
					//$searchArgs=$filter['search_field0']=array("name"=>"1","oper"=>1,"val"=>$srch);
					$searchArgs['search_field0']=array("name"=>"4","oper"=>7,"val"=>$srch);  //Contact Name
					$searchArgs['search_field1']=array("name"=>"5","oper"=>7,"val"=>$srch); //Supplier Name
					$searchArgs['search_field2']=array("name"=>"6","oper"=>7,"val"=>$srch); //Customer PO Number
					$searchArgs['search_field2']=array("name"=>"7","oper"=>7,"val"=>$srch); //Status, Takes Equal to operator
					$searchArgs['search_field2']=array("name"=>"8","oper"=>7,"val"=>$srch); //Plant
					
					 return $searchArgs;
					
				}
			}
			if($Uri_result[2]=='customer_feedback' && $ci->session->getSessionData('complaint_filter')=='search_text')
			{
				//$srch=getUrlParm('kw');
				$srch=$ci->session->getSessionData('search_text');
				if(!empty($srch))
				{
					//$searchArgs=$filter['search_field0']=array("name"=>"1","oper"=>1,"val"=>$srch);
					$searchArgs['search_field0']=array("name"=>"3","oper"=>7,"val"=>$srch); //Contact.
					$searchArgs['search_field1']=array("name"=>"4","oper"=>7,"val"=>$srch); //Sold to customer name
					$searchArgs['search_field2']=array("name"=>"5","oper"=>7,"val"=>$srch); //Ship to customer name.
					$searchArgs['search_field3']=array("name"=>"6","oper"=>7,"val"=>$srch); //Plant.
					$searchArgs['search_field4']=array("name"=>"7","oper"=>7,"val"=>$srch); //Product/Delivery Line Item.
					
					//return $this->setSortArgsColumn($searchArgs,2,null,null);
					//$searchArgs['search_field2']=array("name"=>"6","oper"=>7,"val"=>$srch);
					//echo "<pre>";
					//print_r($searchArgs);
					//echo "</pre>";
					 return $searchArgs;
        ...
        if($org_id && $ci->session->getSessionData('complaint_filter')=='org_id'){
            $searchArgs['search_field0']=array("name"=>"contacts.org_id","oper"=>1,"val"=>$org_id);
        }
        if($ci->session->getSessionData('complaint_filter')=="c_id" && $c_id){
            $searchArgs['search_field0']=array("name"=>"incidents.c_id","oper"=>1,"val"=>$c_id);
        }
        //Condition for action items.
        if(substr_count($_SERVER['REQUEST_URI'],"app/action_items/list") && $ci->session->getSessionData('complaint_filter')=="c_id"){
            $searchArgs['search_field0']=array("name"=>1,"oper"=>1,"val"=>$c_id);    
        }
        if(substr_count($_SERVER['REQUEST_URI'],"app/action_items/list") && $ci->session->getSessionData('complaint_filter')=="org_id"){
            $searchArgs['search_field0']=array("name"=>4,"oper"=>1,"val"=>$org_id);
        }
         
        //$searchArgs = $this->addContactInformation($contactData, $searchArgs, $count);
        //echo "<pre>";
        //echo $ci->session->getSessionData('complaint_filter');
        //print_r($searchArgs);
        //echo "</pre>";
        //exit;
