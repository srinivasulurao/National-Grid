<div id="rn_<?= $this->instanceID ?>" class="<?= $this->classList ?> inv-div" >
		...
		<div class='tabs tab-active' id='tab1' onclick="showTabContent(1)">Investigation</div>
...
ToCustomerName}</td><td>{$del->ShipToCustomerName}</td><td>{$ship_addr}</td><td>{$bill_addr}</td></tr>";
                $counter++;
                endfor;		
				if(!$counter)
