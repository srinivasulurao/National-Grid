...
class CustomerFeedbackSystem extends \RightNow\Models\Base
{
    ...
    public function deliveryLookUp($search_term){
        ...
        if($search_term && $org_id):
            ...
            $sql_instance=RNCPHP\ROQL::query("SELECT * FROM CFS.Delivery WHERE CFS.Delivery.Plant='$org_id' AND CFS.Delivery.Delivery LIKE '%$search_term%' GROUP BY CFS.Delivery.Delivery")->next();
            $data=array();
            $counter=0;
            while($delivery = $sql_instance->next())
            {
  ...
  endwhile;
    ...
    function getDeliveryDetailsData($delivery_no){
        ...
        if($delivery_no){
            ...
            while($delivery = $sql_instance->next())
            {
                $data.="<tr><td>{$delivery['Delivery']}</td><td>{$delivery['DeliveryLineItem']}</td><td>{$delivery['DeliveryGoodsIssueDate']}</td></tr>";
            }
        }

        echo ($data)?"<table width='100%'><tr><th>#</th><th>Product</th><th>Issue Data</th></tr>".$data."</table>":"Sorry, No results found !";
    }
