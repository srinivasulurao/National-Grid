...
function ShowContactName(str)
{
  ...
  xhttp.onreadystatechange = function() {
    if (xhttp.readyState == 4 && xhttp.status == 200) {
		...
		{
			document.getElementById("display").innerHTML = '<ul><li>No Result found</li></ul>';
  ...
  xhttp.open("GET", "/cc/CustomerFeedbackSystem/supplier_search/"+str, true);
  xhttp.send();
}
function fill(str)
{
...
<? if ($this->data['attrs']['label_input']): ?>
	...
	<?php }else{ ?>
	<div class="ui-widget">
    <input type="text" id="contact_name" name="contact_name" class="rn_Text" <?=tabIndex($this->data['attrs']['tabindex'], 1);?> <? if($this->data['attrs']['max_length']): echo('maxlength="' . $this->data['attrs']['max_length'] . '"'); endif; ?> <? if($this->data['cname']){ ?> readonly value="<?=$this->data['cname'];?>" <?php } ?> <? if(empty($this->data['cname'])): ?> onKeyUp="ShowContactName(this.value)" <?php endif; ?> autocomplete="off" />
