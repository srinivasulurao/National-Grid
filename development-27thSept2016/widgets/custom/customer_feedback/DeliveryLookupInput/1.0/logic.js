...
Custom.Widgets.customer_feedback.DeliveryLookupInput = RightNow.Widgets.extend({
    ...
    constructor: function() {
        ...
        YUI().use('event', function (Y) {
            reset_html='<form method="post" onsubmit="return SearchLookUpByEnterButton();"><label>Delivery #</label><input type="text" id="delivery_no" ><label>Customer PO Number</label><input type="text" id="customer_po_no"><label>Sold to Customer Name</label><input type="text" id="sold_to_customer"><label>Ship to Customer Name</label><input type="text" id="ship_to_customer"><input type="submit" value="submit" style="display:none"></form>';
            var ddgb = Y.one("#ddgb");
            ddgb.on("click", function (e) {
//Y.one("#form").setContent("<form method=''>");
                ...
                Y.one("#DeliverySearchPlay").setContent(reset_html);
...
if(document.getElementsByName("Incident.CustomFields.c.product_no").length)
         document.getElementsByName("Incident.CustomFields.c.product_no")[0].className="rn_Text product_no_to_customer";
            ...
            //Check is it a view page.
            var current_url=window.location;
            view_page=current_url.toString().indexOf("view");

            if(parseInt(view_page) > 0){
              document.getElementsByName('Incident.CFS$Delivery')[0].disabled=true;
            }
...
function SearchLookUpByEnterButton(){
	var delivery_no=document.getElementById('delivery_no').value;
    var customer_po_no=document.getElementById('customer_po_no').value;
    var sold_to_customer=document.getElementById('sold_to_customer').value;
    var ship_to_customer=document.getElementById('ship_to_customer').value;
    ...
    var xhttp=new XMLHttpRequest();
	xhttp.open("POST", "/cc/customerFeedbackSystem/deliveryDetailsLookup", false);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("delivery_no="+delivery_no+"&customer_po_no="+customer_po_no+"&sold_to_customer="+sold_to_customer+"&ship_to_customer="+ship_to_customer);

    if(xhttp.responseText){
       document.getElementById('DeliverySearchPlay').innerHTML=xhttp.responseText+"<br>";
       document.getElementById("search_delivery").style.display="none";
       }
    else
       document.getElementById('DeliverySearchPlay').innerHTML=reset_html;
