	...
	protected function filtersToSearchArgs()
    {
      ...
      $report_id=100089;
			...
			if($ci->session->getSessionData('complaint_filter_individual')=='org_id' && $org_id){
					$searchArgs['search_field0']=array("name"=>"incidents.org_id","oper"=>1,"val"=>$org_id);
			}
            ...
            if($ci->session->getSessionData('complaint_status_filter')){
                      ...
                      $searchArgs['search_field1']=array("name"=>"incidents.status_id","oper"=>1,"val"=>$sf_val);
                }

            if($ci->session->getSessionData('complaint_productline_filter')){
                      $val=$ci->session->getSessionData('complaint_productline_filter');
                      $searchArgs['search_field2']=array("name"=>"incidents.prod_id","oper"=>1,"val"=>"$val"); 
            }

			if(!empty($srch) && $ci->session->getSessionData('complaint_filter')=='search_text')
			{
				$columns=array('Contact Name','Sold To Customer','Ship To Customer Name','Subject','Status','Product Line Item','Plant','Reference Number','MaterialDescription','Batch');
				$counter=3;
			...
			if($Uri_result[2]=='investigations')
			{
				...
				if(1){
				  ...
				  $searchArgs=array();
					$filter=$this->getFilterByName($report_id,'Status');
				  $searchArgs['search_field0']=array("name"=>"incidents.c_id","oper"=>1,"val"=>$c_id);
				  $searchArgs['search_field1']=array("name"=>"incidents.status_id","oper"=>1,"val"=>$status_id); //status.
				}

				if($ci->session->getSessionData('complaint_status_filter')){
                          $filter=$this->getFilterByName($report_id,'Status');
                          $sf_val=$ci->session->getSessionData('complaint_status_filter');
                          $searchArgs['search_field2']=array("name"=>"incidents.status_id","oper"=>1,"val"=>$sf_val);
                }
          ...
          $counter=10; //Counter should be distint.
          foreach($columns as $column):
      		  $filter=$this->getFilterByName($report_id,$column);
              $val=($column=="Status")?$status_id:$srch;
              $searchArgs['search_field'.$counter]=array("name"=>$filter->result['fltr_id'],"oper"=>1,"val"=>"%$val%");
              $counter++;
          endforeach;
				}
			}
			...
			if($Uri_result[2]=='action_items')
			{
				...
				$status_id=$ci->model('custom/CustomerFeedbackSystem')->getStatusIdByStatusName($srch); 
				$report_id=100041;
				$columns=array('Description','ID','Category','Group','Contact','Organization','Status','Product');
				...
				if($ci->session->getSessionData('complaint_status_filter')){
					  $filter=$this->getFilterByName($report_id,'Status');
                      $sf_val=$ci->session->getSessionData('complaint_status_filter');
                      $searchArgs['search_field1']=array("name"=>$filter->result['fltr_id'],"oper"=>5,"val"=>$sf_val);
				}

				if($ci->session->getSessionData('complaint_productline_filter')){
					$val=$ci->session->getSessionData('complaint_productline_filter');
					$filter=$this->getFilterByName($report_id,'Product');
					$searchArgs['search_field1']=array("name"=>'CFS$ActionItem.Product',"oper"=>1,"val"=>$val);
				}
