...
class ReportCustomModel extends \RightNow\Models\Report
{
	...
	protected function filtersToSearchArgs()
    {
        ...
        $searchArgs = $this->addContactInformationFilteredByOrganization($contactData, $searchArgs, $count,$org_id);
        }
        else if($ci->session->getSessionData('complaint_filter')=="c_id"){
        $searchArgs = $this->addContactInformationFilteredByContact($contactData, $searchArgs, $count,$c_id);
        }
        else{			
        $searchArgs = $this->addContactInformation($contactData, $searchArgs, $count);
        }
        ...
        if(substr_count($_SERVER['REQUEST_URI'],"app/action_items/list")):
            $searchArgs=array();
        endif;
	...
	function addContactInformationFilteredByOrganization($contactDataSet, $searchArgs, $index = 0,$org_id) {

             //This is a filteration based on the organization id.
             $filter=array();
             $filter['search_field0']=array("name"=>"contacts.org_id","oper"=>1,"val"=>$org_id);
             return $filter;
             
       }
       function addContactInformationFilteredByContact($contactDataSet, $searchArgs, $index = 0,$c_id){
            //This is a filteration based on the organization id.
             $filter=array();
             
             $filter['search_field0']=array("name"=>"incidents.c_id","oper"=>1,"val"=>$c_id);
         
             //$filter['search_field1']=array("name"=>"contacts.c_id","oper"=>1,"val"=>$c_id);
