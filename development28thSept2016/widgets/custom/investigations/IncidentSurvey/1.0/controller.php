...
class IncidentSurvey extends \RightNow\Libraries\Widget\Base {
    ...
    function getData() {
        ...
        if(sizeof($child_incidents) && $child_incidents):
        ...
        if($complaintClosed && $ci->session->getSessionData('last_incident_page_visited')=="update"){
          $this->data['survey_link_name']="Feedback Resolution Survey";
      ...
      if($ci->session->getSessionData('last_incident_page_visited')=="new" || !$complaintClosed){
        // to be shown when the incident is newly created.
          $this->data['survey_link_name']="Risk Rank Survey";
