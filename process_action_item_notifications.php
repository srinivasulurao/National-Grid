...
if(!empty($thirty_day_expire) && !empty($seven_day_expire) && !empty($one_day_expire))
{
	...
	$result = RNCPHP\ROQL::query("select CFS.ActionItem.ID as ActionID,CFS.ActionItem.Description,CFS.ActionItem.DueDate,CFS.ActionItem.Product,CFS.ActionItem.Product.Name as Product,CFS.ActionItem.Location.Name as Location,CFS.ActionItem.Details,CFS.ActionItem.Category.LookupName as Category,CFS.ActionItem.Priority.LookupName as Priority,CFS.ActionItem.Status.LookupName as status,CFS.ActionItem.Contact.ID as c_id  from CFS.ActionItem where (DueDate='$thirty_day_expire' OR DueDate='$seven_day_expire' OR DueDate='$one_day_expire') AND CFS.ActionItem.Status.id !='3'")->next();  
...
function send_nofication_email($result='',$days)
{
	...
	//$contact_email='ranjith.ku.kumar@oracle.com';
	...
	if(!empty($contact_email))
	{
		...
		//set subject
		$mm->Subject = "Incomplete Action Item - due in $days days";
