...
class customDatePickerr extends \RightNow\Libraries\Widget\Input {
    ...
    function getData() {
	  ...
	  $x = explode('.',$this->data['attrs']['name']);
		...
		$this->data['js']['name'] = $x[1];
	   $this->CI->load->model('custom/CustomerFeedbackSystem');
	 $this->data['field'] = $this->CI->CustomerFeedbackSystem->getBusinessObjectFields($this->data['js']['table'],$this->data['js']['name']);	
	  ...
	  if($url_id)
	   {
	   $data_value = $this->CI->CustomerFeedbackSystem->GetTargetDate($url_id);
	   $data_value=date('Y-m-d',$data_value);
		$this->data['field']->value=$data_value;
		$this->data['duedate']= $data_value;
		//$this->data['selected'] = intval($data_value);
	   }
	  else{
      $thirty_days_off=time()+(3600*24*30);
      $this->data['duedate']=date("Y-m-d",$thirty_days_off);
	  }
