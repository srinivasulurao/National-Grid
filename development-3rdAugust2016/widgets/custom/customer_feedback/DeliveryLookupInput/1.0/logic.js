...
Custom.Widgets.customer_feedback.DeliveryLookupInput = RightNow.Widgets.extend({
    ...
    constructor: function() {
                    ...
                    successHandler: function (response) {
                        ...
                        if(response.responseText!=""){
                            ...
                            for (var i = 0; i < arr.length; i++){
                                var obj = arr[i];
                                dd=obj.delivery;
                                html+="<li><a href='javascript:void(0)'  onclick=\"setDelivery('"+dd+"')\">"+obj.delivery+"</a></li>";
                            }
                            html+="</ul></div>";
                        }
                        if(arr.length)
        ...
        YUI().use('event', function (Y) {
            reset_html='<label>Delivery #</label><input type="text" id="delivery_no" ><label>Customer PO Number</label><input type="text" id="customer_po_no"><label>Sold to Customer</label><input type="text" id="sold_to_customer"><label>Ship to Customer</label><input type="text" id="ship_to_customer">';
            var ddgb = Y.one("#ddgb");
            ...
            if(inc_id){
                document.getElementsByClassName("delivery_lookup")[0].disabled="true";
