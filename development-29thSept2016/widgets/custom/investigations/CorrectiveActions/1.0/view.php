<?php if($this->attrs['task']->value=="edit"): ?>
									...
									<div id='correctiveActionList'>
										...
										endforeach;
										if(!$counter)
										echo "<tr><td colspan='5' style='color:red'>Sorry, no corrective actions taken for this investigation.</td></tr>";
										?>
									</table>
									</div>
									...
									<form method='action' id='add_corrective_action' style='display:none' onsubmit="return false;">
										...
										<label>Details</label>
										<textarea style='height:200px' id='corrective_actions_details' required="required"></textarea>
										...
										<button id="editCorrectiveAction" type="submit" style='display:none'>Submit</button>
...
<?php if($this->attrs['task']->value=="view"): ?>
	       <div id="rn_<?= $this->instanceID ?>" class="<?= $this->classList ?> rn_Output" >
					<label>Corrective Actions</label>
							<table class="actiontable" style="width:99%;background:white;color:black;font-size:12px">
							<tr style='background:black !important;color:white'><th>&nbsp Details</th><th>Description</th><th>Completion Status</th><th>Completion Date</th><th>Due Date</th></tr>
					  <?php
					if(sizeof($this->data['corrective_actions'])){
					foreach($this->data['corrective_actions'] as $cs):
						$complete=($cs['Complete'])?"Completed":"Incomplete";
					  $completion_date=($cs['CompletionDate'])?date("m/d/Y",strtotime($cs['CompletionDate'])):"";
						$due_date=($cs['DueDate'])?date("m/d/Y",strtotime($cs['DueDate'])):"";
						echo "<tr><td>&nbsp {$cs['Details']}</td><td>{$cs['Description']}</td><td>$complete</td><td>$completion_date</td><td>$due_date</td></tr>";
					endforeach;
					}
					else{
						echo "<tr><td colspan='5'><font color='red'>&nbsp Sorry, no corrective actions taken for this investigation.</font></td></tr>";
					}
	 ?>
 </table>
	</div>
